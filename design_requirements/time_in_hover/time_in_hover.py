...
for config in configs:
	...
	configs[config]["TOGW"] = np.zeros(np.size(sizing_t_hover_array))
	...
	for i,sizing_t_hover in enumerate(sizing_t_hover_array):
		...
		problem_subDict.update({
			...
			SizingMission.t_hover: sizing_t_hover,#hover time
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(sizing_t_hover_array.to(ureg.s).magnitude,c["TOGW"].to(ureg.lbf).magnitude,
...
title_str = "Aircraft parameters: battery energy density = %0.0f Wh/kg; %0.0f rotor blades; %s\n" \
	...
	+ "Sizing mission (%s): range = %0.0f nmi; %0.0f passengers; reserve type = " \
	% (generic_data["sizing_mission"]["type"], generic_data["sizing_mission"]["range"].to(ureg.nautical_mile).magnitude,\
	 generic_data["sizing_mission"]["N_{passengers}"]) + reserve_type_string + "\n"\

...
sizing_time_in_hover_array = np.linspace(30,240,9)*ureg.s
...
for config in configs:
	...
	configs[config]["MTOW"] = np.zeros(np.size(sizing_time_in_hover_array))
	configs[config]["W_{battery}"] = np.zeros(np.size(sizing_time_in_hover_array))
	configs[config]["cost_per_trip_per_passenger"] = np.zeros(np.size(sizing_time_in_hover_array))
	configs[config]["SPL"] = np.zeros(np.size(sizing_time_in_hover_array))

	for i,sizing_time_in_hover in enumerate(sizing_time_in_hover_array):
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(sizing_time_in_hover_array.to(ureg.s).magnitude,c["MTOW"].to(ureg.lbf).magnitude,
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(sizing_time_in_hover_array.to(ureg.s).magnitude,c["W_{battery}"].to(ureg.lbf).magnitude,
		color="black",linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],
		fillstyle=style["fillstyle"][i],markersize=style["markersize"],label=config)
plt.grid()
plt.ylim(ymin=0)
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(sizing_time_in_hover_array.to(ureg.s).magnitude,c["cost_per_trip_per_passenger"],
		color="black",linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],
		fillstyle=style["fillstyle"][i],markersize=style["markersize"],label=config)
plt.grid()
plt.ylim(ymin=0)
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(sizing_time_in_hover_array.to(ureg.s).magnitude,c["SPL"],
		color="black",linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],
		fillstyle=style["fillstyle"][i],markersize=style["markersize"],label=config)
plt.grid()

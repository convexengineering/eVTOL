# Sweep to evaluate the sensitivity of each design to maximum mean lift coefficient
...
for config in configs:
	...
	if (config == "Helicopter") or (config == "Coaxial heli"):
		Cl_mean_max_array = np.linspace(0.3,0.6,9)
	elif (config == "Autogyro") or (config == "Compound heli"):
		Cl_mean_max_array = np.linspace(0.3,0.8,9)
	else:
		Cl_mean_max_array = np.linspace(0.5,1.5,9)
	
	configs[config]["Cl_mean_max_array"] = Cl_mean_max_array
	configs[config]["TOGW"] = np.zeros(np.size(Cl_mean_max_array))
	configs[config]["W_{battery}"] = np.zeros(np.size(Cl_mean_max_array))
	configs[config]["cost_per_trip_per_passenger"] = np.zeros(np.size(Cl_mean_max_array))
	configs[config]["f_{peak}"] = np.zeros(np.size(Cl_mean_max_array))
	configs[config]["SPL_A"] = np.zeros(np.size(Cl_mean_max_array))

	configs[config]["VT_sizing"] = np.zeros(np.size(Cl_mean_max_array))

	for i,Cl_mean_max in enumerate(Cl_mean_max_array):
		...
		problem_subDict.update({
			...
			Aircraft.rotors.Cl_mean_max: Cl_mean_max, #maximum allowed mean lift coefficient
		...
		configs[config]["VT_sizing"][i] = solution("VT_OnDemandSizingMission")[0].to(ureg.ft/ureg.s).magnitude
	...
	configs[config]["VT_sizing"] = configs[config]["VT_sizing"]*ureg.ft/ureg.s
...
for i, config in enumerate(configs):
	...
	plt.plot(c["Cl_mean_max_array"],c["TOGW"].to(ureg.lbf).magnitude,
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(c["Cl_mean_max_array"],c["cost_per_trip_per_passenger"],
		color="black",linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],
		fillstyle=style["fillstyle"][i],markersize=style["markersize"],label=config)
plt.grid()
[ymin,ymax] = plt.gca().get_ylim()
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(c["Cl_mean_max_array"],c["VT_sizing"].to(ureg.ft/ureg.s).magnitude,color="black",
		linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],fillstyle=style["fillstyle"][i],
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(c["Cl_mean_max_array"],c["SPL_A"],
		color="black",linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],
		fillstyle=style["fillstyle"][i],markersize=style["markersize"],label=config)
plt.grid()
plt.xticks(fontsize=12)

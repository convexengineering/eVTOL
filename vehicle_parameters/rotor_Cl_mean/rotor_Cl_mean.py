...
for config in configs:
	...
	if (config == "Helicopter") or (config == "Coaxial heli"):
		Cl_mean_max = np.linspace(0.3,0.6,9)
	elif (config == "Autogyro") or (config == "Compound heli"):
		Cl_mean_max = np.linspace(0.3,0.8,9)
	else:
		Cl_mean_max = np.linspace(0.5,1.5,9)
	
	Cl_mean_max = ("sweep",Cl_mean_max)
...
Cl_mean_max = Cl_mean_max[1] #convert back to array from tuple
...
for i, config in enumerate(configs):
	...
	plt.plot(Cl_mean,c["MTOW"].to(ureg.lbf).magnitude,
...
for i, config in enumerate(configs):
	c = configs[config]
	Cl_mean = c["solution"]("Cl_{mean_{max}}")
	plt.plot(Cl_mean,c["cost_per_trip_per_passenger"],
		color="black",linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],
		fillstyle=style["fillstyle"][i],markersize=style["markersize"],label=config)
plt.grid()
[ymin,ymax] = plt.gca().get_ylim()
...
for i, config in enumerate(configs):
	...
	plt.plot(Cl_mean,VT,color="black",linewidth=1.5,linestyle=style["linestyle"][i],

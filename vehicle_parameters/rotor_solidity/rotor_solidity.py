...
solidity_array = np.linspace(0.05,0.15,10) #rotor solidity 
...
for config in configs:
	...
	c = configs[config]
	configs[config]["TOGW"] = np.zeros(np.size(solidity_array))
	configs[config]["W_{battery}"] = np.zeros(np.size(solidity_array))
	configs[config]["cost_per_trip_per_passenger"] = np.zeros(np.size(solidity_array))
	configs[config]["f_{peak}"] = np.zeros(np.size(solidity_array))
	configs[config]["SPL_A"] = np.zeros(np.size(solidity_array))
	configs[config]["VT_sizing"] = np.zeros(np.size(solidity_array))

	for i,s in enumerate(solidity_array):
		...
		problem_subDict.update({
			...
			Aircraft.rotors.s: s, #rotor solidity
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(solidity_array,c["TOGW"].to(ureg.lbf).magnitude,
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(solidity_array,c["cost_per_trip_per_passenger"],
		color="black",linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],
		fillstyle=style["fillstyle"][i],markersize=style["markersize"],label=config)
plt.grid()
[ymin,ymax] = plt.gca().get_ylim()
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(solidity_array,c["VT_sizing"].to(ureg.ft/ureg.s).magnitude,color="black",linewidth=1.5,
		linestyle=style["linestyle"][i],marker=style["marker"][i],fillstyle=style["fillstyle"][i],
...
for i, config in enumerate(configs):
	c = configs[config]
	plt.plot(solidity_array,c["SPL_A"],
		color="black",linewidth=1.5,linestyle=style["linestyle"][i],marker=style["marker"][i],
		fillstyle=style["fillstyle"][i],markersize=style["markersize"],label=config)
plt.grid()
plt.ylim(ymin=50)
